...
const LoadingScreen = ({ onLoadingComplete }) => {
  ...
  const shapes = [
        ...
        <div className="absolute inset-0">
          {[...Array(3)].map((_, i) => (
            <div
              ...
              style={{
                animation: `spin 1s linear infinite ${i * -0.33}s`,
        ...
        {[...Array(6)].map((_, i) => (
          <div 
            ...
            style={{
              ...
              animation: `pulse 0.67s infinite ${i * 0.1}s`
            }}
          />
        ))}
        ...
        {[...Array(9)].map((_, i) => (
          <div key={i} className="relative">
            <div className="absolute inset-0 border-[1.5px] border-cyan-500"
                 style={{ animation: `spin 1s infinite ${i * 0.067}s` }}></div>
            <div className="absolute inset-[25%] border-[1.5px] border-cyan-400"
                 style={{ animation: `spin 0.67s infinite ${i * 0.067}s` }}></div>
          </div>
        ))}
      </div>
    ),
          ...
          {[...Array(3)].map((_, i) => (
            <div 
              ...
              style={{
                ...
                clipPath: 'polygon(50% 0%, 100% 100%, 0% 100%)',
                animation: `pulse 0.67s infinite ${i * 0.17}s`
              }}
            />
          ))}
        </div>
          ...
          {[...Array(4)].map((_, i) => (
            <div 
              ...
              style={{ 
                ...
                animation: `ping 0.67s infinite ${i * 0.1}s`
              }}
            />
          ))}
        </div>
    ...
    () => (
      <div className="w-11 h-11 relative opacity-70">
        <div className="absolute inset-0" style={{ animation: 'spin 1s infinite' }}>
          <div className="absolute inset-0 border-3 border-cyan-400 rounded-full" 
               style={{ transform: 'rotate3d(1, 1, 0, 45deg)' }}></div>
        </div>
        <div className="absolute inset-0" style={{ animation: 'spin 1.33s infinite' }}>
        ...
        <div className="absolute inset-0">
          {[...Array(4)].map((_, i) => (
            <div
              ...
              className="absolute inset-0 border-2 border-cyan-400 rounded-full"
              style={{
                animation: `ping 0.5s ease-out infinite ${i * 0.1}s`,
                opacity: 0.7 - (i * 0.2),
  ...
  useEffect(() => {
    ...
    const shapeInterval = 335; // Time per shape (3000ms / 9 shapes ≈ 335ms)
  ...
  return (
    <div className={`fixed inset-0 z-[9999] flex flex-col items-center justify-center bg-black transition-opacity duration-335 ${
          ...
          <div className="relative inline-block">
            <div className="relative">
              <img 
                src="/load.png" 
                alt="Retro Computer" 
                className={`h-32 w-32 transition-opacity duration-335 drop-shadow-[0_0_15px_rgba(20,184,166,0.7)] animate-pulse ${
                  imageVisible ? 'opacity-100' : 'opacity-0'
                }`}
              />
              <div 
                className={`absolute flex items-center justify-center transition-opacity duration-335 ${
                  imageVisible ? 'opacity-100' : 'opacity-0'
                }`}
                style={{
                  top: '30%',
